...
def extract_urls_from_tool_response(tool_response: str, urls_to_send: list, seen_urls: set) -> None:
    try:
...
def resolve_mentions_in_message(content: str, mentions, role_mentions, channel_mentions) -> str:
...
def parse_reply_tags(text: str):
    ...
    reply_to = None
    reply_to_match = re.match('\\[REPLY_TO:([^\\]]+)\\](.*)', text, re.DOTALL | re.IGNORECASE)
    if reply_to_match:
