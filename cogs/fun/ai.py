...
class FunAI(commands.Cog):
    ...
    async def _call_tool(
        ...
        try:
            if tool_name == "get_random_cat":
                ...
                # If it's already a full custom emoji string (starts with < and ends with >), use it directly
                if not (emoji_input.startswith("<") and emoji_input.endswith(">")):
                    # If we have an emoji map for this server, try to resolve emoji name
                ...
                try:
                    ...
                    info = {
                        ...
                        "tag": str(member),
                        "id": str(member.id),
                        "joined_at": str(member.joined_at.strftime("%B %d, %Y"))
                        if member.joined_at
                        else "Unknown",
                ...
                async with session.get(
                    ...
                    if data.get("list"):
                        entry = data["list"][0]
                        definition = re.sub(r"\[|\]", "", entry.get("definition", ""))[
                            :400
                        ]
                ...
                return json.dumps(
                    {
                        ...
                        "has_owners": len(owner_ids_list) > 0,
                        "has_admins": len(admin_ids_list) > 0,
                        "has_mods": len(mod_ids_list) > 0,
                    }
                )
                ...
                report_message += (
                    f"**Reported by:** {message.author} (ID: {message.author.id})\n"
                )
                report_message += (
                ...
                # Send DMs to all report targets
                dm_errors = []
    ...
    async def _generate_response(
        ...
        # Use provided temperature or random default
        ...
        try:
            if use_tools:
                try:
                    ...
                    # Check for old <function=...> syntax in the content
                    has_old_function_syntax = False
                    old_function_calls = []
                    if msg.content:
                        old_function_calls = self._parse_old_function_syntax(
                            msg.content
                        )
            ...
            # Fallback without tools (but still check for <function=...> syntax)
            completion = await client.chat.completions.create(
                model=model,
                messages=messages,
                temperature=temperature,
            ...
            old_function_calls = (
                self._parse_old_function_syntax(response_text) if response_text else []
            )
    ...
    def _should_reply(self, message: discord.Message, bot_mentioned: bool) -> bool:
        ...
        if bot_mentioned:
            return True
        # If someone says something ridiculous or we have a savage reply, reply more often
        if elapsed < 10:
            return False
        if msg_count < 3:
            return False
        if elapsed > 120:  # Reply faster than 5 mins
            return True

        # Higher chance to reply than before
        ...
        if guild_context:
            ...
            for msg in guild_context:
                prefix = (
                    "you"
                    if msg["is_bot"]
                    else f"{msg['author_name']} (ID:{msg['author_id']})"
                )
                lines.append(f"{prefix}: {msg['content']}")
    ...
    def _parse_reply_tags(self, text: str):
        ...
        delay_match = re.match(
            r"\[DELAY:(\d+)([sm])\](.*)", text, re.DOTALL | re.IGNORECASE
        )
        if delay_match:
        ...
        send_match = re.match(
            r"\[(REPLY_MENTION|REPLY|SEND|SEND_REPLY)\](.*)",
            text,
        ...
        reaction_match = re.match(
            r"\[REACT:([^\]]+)\](.*)", text, re.DOTALL | re.IGNORECASE
        )
        if reaction_match:
    ...
    async def on_message(self, message: discord.Message):
        ...
        async def handle_reply():
            try:
                ...
                # Helper function to convert :emoji_name: to custom emojis in response text and fix issues
                def replace_emojis(text):
                    if not text:
                        return text
                        
                    # Remove any "IIIK" or similar garbage
                    text = re.sub(r"IIIK", "", text)
                    
                    # Fix broken emojis that are just <number>
                    ...
                    text = re.sub(r"<(\d+)>", fix_broken_emoji, text)
                    
                    # Replace :emoji_name: with custom emojis
                    if hasattr(self, "current_emoji_map"):
                ...
                # Collect server emojis as usable emoji strings
                emoji_list = []
                emoji_map = {}  # Map emoji name to usable emoji string
...
"""
                ...
                # Random temperature for more varied responses
                temperature = random.uniform(1.2, 1.8)
                ...
                delay_seconds, send_type, reply_text, reaction_emoji = (
                    self._parse_reply_tags(reply)
                )

                # Helper function to properly resolve reaction emoji
                def resolve_reaction_emoji(emoji_str):
                    if not emoji_str:
                        return None
                    emoji_str = emoji_str.strip()
                    # If it's already a full custom emoji string, use it directly
                    if emoji_str.startswith("<") and emoji_str.endswith(">"):
                        return emoji_str
                    # If it's :emoji_name: format, try to resolve
                    ...
                    # Otherwise, assume it's a Unicode emoji
                    return emoji_str
                
                # Replace custom emoji mentions in reply text and reaction
                            ...
                            if len(recent) >= 8:
                                ...
                                self.bot.loop.create_task(
                                    self._summarize_memory(
                                        message.author.id, to_summarize
                                    )
                                )
                ...
                if message.author.id in self._pending_summarize:
                    ...
                    if len(recent) >= 8:
                        ...
                        remaining = recent[6:]
                        self.bot.loop.create_task(
                            self._summarize_memory(message.author.id, to_summarize)
                        )
                        self.user_memory_recent[message.author.id] = deque(
                            remaining, maxlen=10
                        )
    ...
    async def insult(self, ctx: commands.Context, member: discord.Member):
        ...
        await ctx.defer()
        response, urls = await self._generate_response(
            "Write a harmless, clearly jokey, exaggerated fake insult. Make it absurd enough that no one could take it seriously. Max 300 chars. No markdown.",
            f"Write a silly fake insult for {member.display_name}.",
            message=ctx.message,
        )
        await ctx.send(f"{member.mention} {response}")
        for url in urls:
    ...
    async def translate(self, ctx: commands.Context, language: str, *, text: str):
        ...
        result, _ = await self._call_tool(
    ...
    async def github(self, ctx: commands.Context, username: str):
        ...
        try:
            ...
            if "login" in data:
                ...
                await ctx.send(
                    f"**{data['login']}**{' (' + data['name'] + ')' if data.get('name') else ''}{bio}\n"
                    f"📦 {data['public_repos']} repos | 👥 {data['followers']} followers | joined {data['created_at'][:7]}{location}"
                )
            else:
    ...
    @commands.hybrid_command(
        name="anime_quote", description="Get a random anime quote!"
    )
    ...
    @commands.hybrid_command(
        name="clear_memory", description="Clear your personal chat memory with the bot!"
    )
    ...
    @commands.hybrid_command(
        name="view_memory", description="View your personal chat memory with the bot!"
    )
