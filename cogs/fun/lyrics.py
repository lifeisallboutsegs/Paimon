    ...
    async def lyrics_command(self, ctx: commands.Context, *, query: str):
        ...
        for i, hit in enumerate(hits[:10]):
            ...
            options.append(
                discord.SelectOption(
                    label=label[:100], value=str(i), description=result["url"][:100]
                )
            )
            lines.append(
                f"`{i + 1}.` **{result['title']}** — {result['primary_artist']['name']}"
            )
        ...
        class SongSelect(discord.ui.View):
            ...
            async def select_callback(
                ...
                art_file = (
                    discord.File(io.BytesIO(art_data), filename="cover.jpg")
                    if art_data
                    else None
                )
                await self.message.edit(
                    content=chunks[0],
                    attachments=[art_file] if art_file else [],
                    view=None,
                )
