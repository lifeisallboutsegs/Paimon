...
class ModerationPunishments(commands.Cog):
    ...
    @app_commands.describe(user="User to unban (username or ID)")
    @mod_role_or_permission("ban_members")
    @commands.bot_has_permissions(ban_members=True)
    ...
    @app_commands.describe(
        member="Member to timeout", duration="e.g. 10m, 1h, 1d", reason="Reason"
    )
    ...
    async def timeout(
        ...
        *,
        ...
        await ctx.send(
            f"✅ Member Timed Out: {member.mention} for {helpers.human_timedelta_seconds(seconds)} — {reason}"
        )

    @commands.hybrid_command(
        name="mute", description="Mute a member using the Muted role."
    )
