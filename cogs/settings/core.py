...
class SettingsCore(commands.Cog):
    ...
    @commands.hybrid_group(
        name="config", description="View or change server configuration."
    )
    async def config(self, ctx: commands.Context):
        cfg = await self.bot.db.get_guild_config(ctx.guild.id)
        welcome_channel = (
            f"<#{cfg['welcome_channel']}>" if cfg.get("welcome_channel") else "Not set"
        )
        log_channel = (
    ...
    @config.command(
        name="welcome_channel", description="Set the welcome message channel."
    )
