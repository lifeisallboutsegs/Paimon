...
class LevelingCore(commands.Cog):
    ...
    async def on_message(self, message: discord.Message):
        ...
        result = await self.bot.db.add_xp(
            message.guild.id, message.author.id, xp_amount
        )
                ...
                if str(new_level) in rewards:
                        ...
                        if role and role not in message.author.roles:
                            try:
                                await message.author.add_roles(
                                    role, reason=f"Level {new_level} reward!"
                                )
                    ...
                    if reward.get("coins") and reward["coins"] > 0:
                        await self.bot.db.add_balance(
                            message.guild.id, message.author.id, reward["coins"]
                        )
    ...
    @commands.hybrid_command(
        name="level", description="Check your or someone else's level and XP."
    )
    ...
    @commands.hybrid_command(
        name="leaderboard", description="Show the server's level leaderboard."
    )
    async def leaderboard(self, ctx: commands.Context):
        ...
        if not data:
            ...
            for i, entry in enumerate(data, 1):
                ...
                msg.append(
                    f"**{i}.** {name} - Level {entry['level']} ({entry['xp']} XP"
                )
        await ctx.send("\n".join(msg))
