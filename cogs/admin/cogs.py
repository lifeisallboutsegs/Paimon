...
class AdminCogs(commands.Cog):
    ...
    async def _suggest_cog_info(self, content: str, valid_categories: list, name: str=None, category: str=None):
        ...
        try:
            completion = await client.chat.completions.create(model='llama-3.3-70b-versatile', messages=[{'role': 'system', 'content': system_prompt}, {'role': 'user', 'content': f'Content sample:\n{content_sample}'}], temperature=0.3, max_completion_tokens=200)
            import json
