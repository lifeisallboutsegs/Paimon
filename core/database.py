...
class Database:
    ...
    async def set_guild_config(self, guild_id: int, **fields) -> None:
        await self.execute(
            "INSERT INTO guild_configs (guild_id) VALUES (?) ON CONFLICT(guild_id) DO NOTHING",
            (guild_id,),
        )
        if not fields:
            return
    ...
    async def add_balance(self, guild_id: int, user_id: int, amount: int) -> int:
        await self.execute(
            "INSERT INTO economy (guild_id, user_id, balance) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET balance = balance + ?",
    ...
    async def set_last_daily(
        ...
        await self.execute(
            "INSERT INTO economy (guild_id, user_id, last_daily) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET last_daily = ?",
    ...
    async def set_last_work(
        ...
        await self.execute(
            "INSERT INTO economy (guild_id, user_id, last_work) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET last_work = ?",
            (guild_id, user_id, iso_timestamp, iso_timestamp),
        )
    ...
    async def get_level(self, guild_id: int, user_id: int) -> dict:
        ...
        return (
            {"xp": row["xp"], "level": row["level"]} if row else {"xp": 0, "level": 0}
        )
    ...
    async def kv_set(self, namespace: str, key: str, value: str) -> None:
        await self.execute(
            "INSERT INTO kv_store (namespace, key, value) VALUES (?, ?, ?) ON CONFLICT(namespace, key) DO UPDATE SET value = ?",
