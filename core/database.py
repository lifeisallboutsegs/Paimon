...
class Database:
    ...
    async def get_guild_config(self, guild_id: int) -> dict:
        ...
        if row is None:
            return {'guild_id': guild_id, 'prefix': None, 'welcome_channel': None, 'log_channel': None, 'mod_role': None}
        return dict(row)

    async def set_guild_config(self, guild_id: int, **fields) -> None:
        await self.execute('INSERT INTO guild_configs (guild_id) VALUES (?) ON CONFLICT(guild_id) DO NOTHING', (guild_id,))
        if not fields:
            return
        columns = ', '.join((f'{k} = ?' for k in fields))
        await self.execute(f'UPDATE guild_configs SET {columns} WHERE guild_id = ?', (*fields.values(), guild_id))

    async def add_warning(self, guild_id: int, user_id: int, moderator_id: int, reason: str) -> int:
        cursor = await self._conn.execute('INSERT INTO warnings (guild_id, user_id, moderator_id, reason) VALUES (?, ?, ?, ?)', (guild_id, user_id, moderator_id, reason))
        await self._conn.commit()
        return cursor.lastrowid

    async def get_warnings(self, guild_id: int, user_id: int) -> list[aiosqlite.Row]:
        return await self.fetchall('SELECT * FROM warnings WHERE guild_id = ? AND user_id = ? ORDER BY id DESC', (guild_id, user_id))
    ...
    async def get_balance(self, guild_id: int, user_id: int) -> int:
        row = await self.fetchone('SELECT balance FROM economy WHERE guild_id = ? AND user_id = ?', (guild_id, user_id))
        return row['balance'] if row else 0

    async def add_balance(self, guild_id: int, user_id: int, amount: int) -> int:
        await self.execute('INSERT INTO economy (guild_id, user_id, balance) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET balance = balance + ?', (guild_id, user_id, amount, amount))
    ...
    async def set_last_daily(self, guild_id: int, user_id: int, iso_timestamp: str) -> None:
        await self.execute('INSERT INTO economy (guild_id, user_id, last_daily) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET last_daily = ?', (guild_id, user_id, iso_timestamp, iso_timestamp))

    async def get_last_daily(self, guild_id: int, user_id: int) -> str | None:
        row = await self.fetchone('SELECT last_daily FROM economy WHERE guild_id = ? AND user_id = ?', (guild_id, user_id))
        return row['last_daily'] if row else None

    async def get_last_work(self, guild_id: int, user_id: int) -> str | None:
        row = await self.fetchone('SELECT last_work FROM economy WHERE guild_id = ? AND user_id = ?', (guild_id, user_id))
        return row['last_work'] if row else None

    async def set_last_work(self, guild_id: int, user_id: int, iso_timestamp: str) -> None:
        await self.execute('INSERT INTO economy (guild_id, user_id, last_work) VALUES (?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET last_work = ?', (guild_id, user_id, iso_timestamp, iso_timestamp))

    async def get_balance_leaderboard(self, guild_id: int, limit: int=10) -> list[aiosqlite.Row]:
        return await self.fetchall('SELECT user_id, balance FROM economy WHERE guild_id = ? ORDER BY balance DESC LIMIT ?', (guild_id, limit))

    async def add_xp(self, guild_id: int, user_id: int, amount: int, cooldown_seconds: int=60) -> tuple[bool, int] | None:
        """Add XP, respecting a per-message cooldown. Returns None if still on cooldown,
        otherwise (leveled_up, new_level)."""
        now = datetime.now(timezone.utc)
        row = await self.fetchone('SELECT xp, level, last_xp_at FROM levels WHERE guild_id = ? AND user_id = ?', (guild_id, user_id))
        ...
        while xp >= xp_for_level(level):
            ...
            level += 1
            leveled_up = True
        await self.execute('INSERT INTO levels (guild_id, user_id, xp, level, last_xp_at) VALUES (?, ?, ?, ?, ?) ON CONFLICT(guild_id, user_id) DO UPDATE SET xp = ?, level = ?, last_xp_at = ?', (guild_id, user_id, xp, level, now.isoformat(), xp, level, now.isoformat()))
        return (leveled_up, level)

    async def get_level(self, guild_id: int, user_id: int) -> dict:
        row = await self.fetchone('SELECT xp, level FROM levels WHERE guild_id = ? AND user_id = ?', (guild_id, user_id))
    ...
    async def get_level_leaderboard(self, guild_id: int, limit: int=10) -> list[aiosqlite.Row]:
        return await self.fetchall('SELECT user_id, xp, level FROM levels WHERE guild_id = ? ORDER BY level DESC, xp DESC LIMIT ?', (guild_id, limit))

    async def kv_set(self, namespace: str, key: str, value: str) -> None:
        await self.execute('INSERT INTO kv_store (namespace, key, value) VALUES (?, ?, ?) ON CONFLICT(namespace, key) DO UPDATE SET value = ?', (namespace, key, value, value))

    async def kv_get(self, namespace: str, key: str) -> str | None:
        row = await self.fetchone('SELECT value FROM kv_store WHERE namespace = ? AND key = ?', (namespace, key))
