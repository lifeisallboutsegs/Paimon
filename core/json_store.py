...
class JSONDatabase:
    ...
    def __init__(self, directory: Path):
        ...
        self._files = {'guild_configs': self.directory / 'guild_configs.json', 'warnings': self.directory / 'warnings.json', 'economy': self.directory / 'economy.json', 'levels': self.directory / 'levels.json', 'kv_store': self.directory / 'kv_store.json'}
    ...
    async def get_guild_config(self, guild_id: int) -> dict:
        async with self._lock:
            ...
            return data.get(str(guild_id), {'guild_id': guild_id, 'prefix': None, 'welcome_channel': None, 'log_channel': None, 'mod_role': None})
    ...
    async def add_warning(self, guild_id: int, user_id: int, moderator_id: int, reason: str) -> int:
        async with self._lock:
            ...
            warnings.append({'id': new_id, 'guild_id': guild_id, 'user_id': user_id, 'moderator_id': moderator_id, 'reason': reason, 'created_at': datetime.now(timezone.utc).isoformat()})
    ...
    async def get_balance_leaderboard(self, guild_id: int, limit: int=10) -> list[dict]:
        async with self._lock:
            data = self._read('economy')
            rows = []
            for key, entry in data.items():
                gid, uid = key.split(':')
                if int(gid) == guild_id:
                    rows.append({'user_id': int(uid), 'balance': entry.get('balance', 0)})
    ...
    async def get_level_leaderboard(self, guild_id: int, limit: int=10) -> list[dict]:
        async with self._lock:
            ...
            for key, entry in data.items():
                gid, uid = key.split(':')
                if int(gid) == guild_id:
                    rows.append({'user_id': int(uid), 'xp': entry['xp'], 'level': entry['level']})
