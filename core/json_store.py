...
class JSONDatabase:
    ...
    async def close(self) -> None:
        pass  # Every write below flushes immediately; nothing to clean up.
    ...
    async def add_warning(self, guild_id: int, user_id: int, moderator_id: int, reason: str) -> int:
        async with self._lock:
            ...
            warnings.append({
                "id": new_id, "guild_id": guild_id, "user_id": user_id,
    ...
    async def clear_warnings(self, guild_id: int, user_id: int) -> None:
        async with self._lock:
            warnings = self._read("warnings")
            warnings = [
                w for w in warnings if not (w["guild_id"] == guild_id and w["user_id"] == user_id)
            ]
            self._write("warnings", warnings)
